...
def predict_number(x):
    ...
    result = np.argmax(model.predict(x, verbose = 0))
    return result

#%%

def printFont(string , color_tuple , size , pos_tuple):
    fontObj = pygame.font.Font('MaplestoryLight.ttf' , size)
...
#        pygame.draw.rect(screen, semiBLACK, rect)
    ...
    printFont('지', WHITE, 32, (width - extra_wid + 35,40))
    printFont('우', WHITE, 32, (width - extra_wid + 35,80))
    printFont('개', WHITE, 32, (width - extra_wid + 35,120))
    # 글자 쓰기
    
    printFont(str(num), BLACK, 32, (width - extra_wid + 35,190))
